...
for j in range(DATA_SIZE):
    for i in range(int(SAMPLE_SIZE/DATA_SIZE)):
            amp_modulation = data[j]*math.sin(math.pi*i/50)*AM_DEPTH + math.sin(math.pi*i/50)*(1-AM_DEPTH)
            phase_modulation = math.sin(math.pi*i/50+(1-data[j])*math.pi*PM_DEPTH)
            amp_modulated.append(amp_modulation)
            phase_modulated.append(phase_modulation)
...
for i in range(SAMPLE_SIZE):
    ...
    carrier.append(math.sin(math.pi*i/50+PHASE_ERROR))
